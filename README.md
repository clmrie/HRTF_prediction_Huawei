...
# Seeing the Sound of an Ear

### Personalized HRTF prediction from a handful of pinna photographs
...
<p align="center">
  <img src="assets/fig1_teaser.svg" width="100%" alt="A few ear photos go into one network and come out as a full-sphere personalized HRTF."/>
</p>

## What this does

Spatial audio sounds convincing only when it is filtered by **your** ears. That filter — the **Head-Related Transfer Function (HRTF)** — is normally measured in a treated room with a loudspeaker arc and in-ear microphones, which takes tens of minutes per person and confines personalization to the lab.

We predict it **from photographs instead**. A single network takes a few grayscale photos of both ears and outputs the listener's complete HRTF — **793 directions × 2 ears**, written straight to a SOFA file. No 3-D scan, no acoustic simulation, no tape measure, no listening test.
...
**Decoding the whole sphere at once.** Instead of querying one direction at a time like neural-field approaches, an **expansion LSTM** unrolls the 2048-dim identity code into all 1,586 output channels, and a shared MLP decodes each into a 256-tap impulse response. This costs **≈21M parameters** where a dense head would need **≈832M**.

**The metric is the loss.** The challenge's scoring function is already differentiable, so we optimize it directly rather than a proxy — every gradient step descends the leaderboard criterion.

**Augmentation that respects the multi-view structure.** Augmentation parameters are drawn once per *subject* and applied identically to every view, so the cross-view consistency the aggregator depends on survives.

## The objective, and its blind spot
...
**Read this number carefully.** Per-task (19/7/3-view) scores were not retained, so it can't be decomposed by regime, and we no longer have the population-average baseline for comparison. Model selection and scoring both used the same 10 evaluation subjects, so it is a **best-checkpoint figure, not a clean held-out estimate**. MSD is also not comparable to the log-spectral-distortion numbers (typically 3–6 dB) reported elsewhere — different scale entirely.
...
- **Controlled capture.** All inputs come from SONICOM's capture rig — phone photos, bad lighting, and hair occlusion are untested.
...
[10] Y. Zhang, Y. Wang, Z. Duan. HRTF Field: neural fields for HRTF magnitude. *IEEE ICASSP*, 2023. — plus NIIRF (ICASSP 2024) and RANF (ICASSP 2025).<br>
